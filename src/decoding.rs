...
use {
    BroadcastMode, Instruction, MaskReg, MergeMode, Mode, Operand, OperandSize, Reg, RegScale,
    RegType, RoundingMode,
};
...
impl<T: Read> InstructionReader<T> {
    ...
    pub fn read(&mut self) -> Result<Instruction, InstructionDecodingError> {
        ...
        loop {
            ...
            match b {
                PREFIX_LOCK => {
                    buffer.prefix1 = Some(Prefix1::Lock);
                }
                PREFIX_REPNE => {
                    buffer.prefix1 = Some(Prefix1::RepNE);
                    buffer.f2_prefix = true;
                }
                PREFIX_REP => {
                    buffer.prefix1 = Some(Prefix1::Rep);
                    buffer.f3_prefix = true;
                }
                ...
                PREFIX_CS => {
                    buffer.prefix2 = Some(Prefix2::CS);
                }
                PREFIX_SS => {
                    buffer.prefix2 = Some(Prefix2::SS);
                }
                PREFIX_DS => {
                    buffer.prefix2 = Some(Prefix2::DS);
                }
                PREFIX_ES => {
                    buffer.prefix2 = Some(Prefix2::ES);
                }
                PREFIX_FS => {
                    buffer.prefix2 = Some(Prefix2::FS);
                }
                PREFIX_GS => {
                    buffer.prefix2 = Some(Prefix2::GS);
                }
                PREFIX_TWO_BYTE_OPCODE => {
                ...
                PREFIX_VEX2 => {
                    ...
                    reg_ext = if data & 0x80 != 0 || self.mode != Mode::Long {
                        0
                    } else {
                        0x8
                    };
                    buffer.vex_operand =
                        Some((!data >> 3) & if self.mode == Mode::Long { 0xF } else { 0x7 });
                ...
                PREFIX_VEX3 => {
                    ...
                    buffer.composite_prefix = Some(CompositePrefix::Vex);
                    reg_ext = if data1 & 0x80 != 0 || self.mode != Mode::Long {
                        0
                    } else {
                        0x8
                    };
                    index_ext = if data1 & 0x40 != 0 || self.mode != Mode::Long {
                        0
                    } else {
                        0x8
                    };
                    b_ext = if data1 & 0x20 != 0 || self.mode != Mode::Long {
                        0
                    } else {
                        0x8
                    };
                    match data1 & 0x1F {
                        ...
                        2 => {
                            buffer.is_two_byte_opcode = true;
                            buffer.primary_opcode = 0x38;
                        }
                        3 => {
                            buffer.is_two_byte_opcode = true;
                            buffer.primary_opcode = 0x3A;
                        }
                        _ => return Err(InstructionDecodingError::InvalidInstruction),
                    }
                    buffer.vex_e = Some(data2 & 0x80 != 0);
                    buffer.vex_operand =
                        Some((!data2 >> 3) & if self.mode == Mode::Long { 0xF } else { 0x7 });
                ...
                PREFIX_EVEX
                    if self.mode == Mode::Long || lookahead.map_or(false, |l| l & 0xC0 == 0xC0) =>
                {
                    let data1 = self.expect_byte()?;
                    let data2 = self.expect_byte()?;
                    ...
                    reg_ext |= if data1 & 0x10 == 0 && self.mode == Mode::Long {
                        0x10
                    } else {
                        0
                    };
                    ...
                    buffer.merge_mode = Some(if data3 & 0x80 != 0 {
                        MergeMode::Zero
                    } else {
                        MergeMode::Merge
                    });
                ...
                b if self.mode == Mode::Long
                    && buffer.composite_prefix.is_none()
                    && b & 0xF0 == 0x40 =>
                {
                    // REX prefix
                ...
                b => {
                    opcode_byte = b;
                    break;
                } // Not a prefix, move on
            }
        }

        let addr_mode = Mode::from_size(InstructionReader::<T>::get_addressing_mode(
            self.mode, &buffer,
        ))
        .unwrap();
        ...
        if (buffer.primary_opcode == 0x38 || buffer.primary_opcode == 0x3A)
            && buffer.secondary_opcode.is_none()
        {
        ...
        if def_res
            .map(|def| InstructionReader::<T>::has_mod_rm(def))
            .unwrap_or(false)
        ...
        // we read.
        let def = def_res.or_else(|_| {
            find_instruction_def_by_opcode(&buffer, self.mode)
        ...
        let operand_results: Result<Vec<Operand>, InstructionDecodingError> = def
            .operands
            .iter()
            .filter_map(|maybe_op_def| {
                maybe_op_def
                    .as_ref()
                    .map(|op_def| self.read_operand(op_def, &buffer))
            })
            .collect();
        ...
        Ok(Instruction {
            ...
            merge_mode: if def.allow_merge_mode && buffer.merge_mode != Some(MergeMode::Merge) {
                buffer.merge_mode
            } else {
                None
            },
        ...
        Ok(match op_def.encoding {
            OperandEncoding::ModRmReg => {
                if let OperandType::Reg(reg_type) = op_def.op_type {
                    Operand::Direct(
                        Reg::from_code_reg_type(
                            buffer.mod_rm_reg.unwrap(),
                            reg_type,
                            size,
                            buffer.has_rex(),
                        )
                    ...
                    panic!("Invalid operand definition.");
                }
            }

            OperandEncoding::ModRmRm => {
                // TODO Handle MIB
                let reg_type = if let OperandType::Reg(reg_type) = op_def.op_type {
                    reg_type
                } else if let OperandType::Set(set) = op_def.op_type {
                    set.iter()
                        .filter_map(|i| {
                            if let OperandType::Reg(reg_type) = *i {
                                Some(reg_type)
                            } else {
                                None
                            }
                        ...
                        .next()
                        .unwrap_or(RegType::General)
                } else {
                    RegType::General
                };
                self.rm_helper(buffer, op_def, |c| {
                    Reg::from_code_reg_type(c, reg_type, size, buffer.has_rex())
                })?
            }
            ...
            OperandEncoding::Imm => match op_def.op_type {
                OperandType::Reg(reg_type) => Operand::Direct(
                    Reg::from_code_reg_type(
                        self.expect_byte()? >> 4,
                        reg_type,
                        size,
                ...
                OperandType::Imm => match op_def.size {
                    ...
                    OperandSize::Word => (0..2)
                        .fold(Ok(0), |acc, n| {
                            acc.and_then(|a| {
                                self.expect_byte().map(|b| a | ((b as u16) << (8 * n)))
                            })
                        })
                        .map(|b| Operand::Literal16(b))?,
                    OperandSize::Dword => (0..4)
                        .fold(Ok(0), |acc, n| {
                            acc.and_then(|a| {
                                self.expect_byte().map(|b| a | ((b as u32) << (8 * n)))
                            })
                        })
                        .map(|b| Operand::Literal32(b))?,
                    OperandSize::Far16 => {
                        // 16:16
                        let addr = (0..2).fold(Ok(0), |acc, n| {
                            acc.and_then(|a| {
                                self.expect_byte().map(|b| a | ((b as u16) << (8 * n)))
                            })
                        })?;
                        let segment = (0..2).fold(Ok(0), |acc, n| {
                            acc.and_then(|a| {
                                self.expect_byte().map(|b| a | ((b as u16) << (8 * n)))
                            })
                        })?;
                ...
                OperandType::Rel(_) => Operand::Offset(
                    (0..op_def.size.bits() >> 3).fold(Ok(0), |acc, n| {
                        acc.and_then(|a| self.expect_byte().map(|b| a | ((b as u64) << (8 * n))))
                    })?,
                    None,
                    None,
                ),
            ...
            OperandEncoding::Offset => Operand::Offset(
                match addr_size {
            ...
            OperandEncoding::Fixed => match op_def.op_type {
                OperandType::Fixed(FixedOperand::Reg(reg)) => Operand::Direct(reg),
                OperandType::Fixed(FixedOperand::Constant(v)) => match op_def.size {
                    OperandSize::Unsized | OperandSize::Byte => Operand::Literal8(v as u8),
    ...
    fn read_disp16(&mut self) -> Result<u16, InstructionDecodingError> {
        (0..2).fold(Ok(0), |acc, n| {
            acc.and_then(|a| self.expect_byte().map(|b| a | ((b as u16) << (8 * n))))
        })
    }

    fn read_disp32(&mut self) -> Result<u32, InstructionDecodingError> {
        (0..4).fold(Ok(0), |acc, n| {
            acc.and_then(|a| self.expect_byte().map(|b| a | ((b as u32) << (8 * n))))
        })
    }
    ...
    fn has_mod_rm(def: &InstructionDefinition) -> bool {
        def.opcode_ext.is_some()
            || def.operands.iter().any(|o| {
                o.as_ref().map_or(false, |op| match op.encoding {
                    OperandEncoding::ModRmReg | OperandEncoding::ModRmRm | OperandEncoding::Mib => {
                        true
                    }
                    _ => false,
                })
    ...
    fn get_address_size(mode: Mode, buffer: &InstructionBuffer) -> OperandSize {
        match (mode, buffer.address_size_prefix) {
            ...
            (Mode::Real, true) | (Mode::Protected, false) | (Mode::Long, true) => {
                OperandSize::Dword
            }
        ...
        (mode != Mode::Real)
            && (buffer
                ...
                .and_then(|rm_mod| {
                    buffer
                        .mod_rm_rm
                        .map(|rm_rm| (rm_rm & 0b111) == 0b100 && rm_mod != 0b11)
                })
                .unwrap_or(false))
    }
        ...
        TConv: Fn(u8) -> Option<Reg>,
    {
        ...
        Ok(match addr_size {
            OperandSize::Word => {
                ...
                let disp = if mode == 0 && rm != 0b110 || mode == 3 {
                    None
                }
                // No displacement
                else {
                    ...
                    Some(if mode == 1 {
                        self.read_disp8()? as u64
                    } else {
                        self.read_disp16()? as u64
                    })
                };
                ...
                match (reg1, reg2, disp) {
                    ...
                    (Some(r1), Some(r2), Some(disp)) => Operand::IndirectScaledIndexedDisplaced(
                        r1,
                        r2,
                        RegScale::One,
                        disp,
            ...
            addr_size @ OperandSize::Dword | addr_size @ OperandSize::Qword => {
                    ...
                    & 0x7
                {
                    0b00 => {
                        match rm {
                            0b000 | 0b001 | 0b010 | 0b011 | 0b110 | 0b111 =>
                            // [RM]
                            {
                                Operand::Indirect(
                                    Reg::from_code_general_sized(
                                        rm,
                            ...
                            0b101 => {
                                if addr_size == OperandSize::Dword {
                                    Operand::Memory(self.read_disp32()? as u64, size, segment)
                                } else {
                                    Operand::Offset(self.read_disp32()? as u64, size, segment)
                                }
                            }
                            _ => unreachable!(),
                        }
                    ...
                    0b11 => Operand::Direct(
                        conv_proc(rm).ok_or(InstructionDecodingError::InvalidInstruction)?,
                    ),
    ...
    fn sib_helper(
        ...
        let scale = buffer
            .sib_scale
            .and_then(|scale| RegScale::from_sib_code(scale))
            .ok_or(InstructionDecodingError::InvalidInstruction)?;
        let index_code = buffer
            .sib_index
            .ok_or(InstructionDecodingError::InvalidInstruction)?;
        let index = Reg::from_code_general_sized(
            index_code,
            InstructionReader::<T>::has_rex(buffer),
            addr_size,
        )
        .ok_or(InstructionDecodingError::InvalidInstruction)?;
        let base_code = buffer
            .sib_base
            .ok_or(InstructionDecodingError::InvalidInstruction)?;
        let base = Reg::from_code_general_sized(
            base_code,
            InstructionReader::<T>::has_rex(buffer),
            addr_size,
        )
    ...
    fn get_operand_type<'a>(
        ...
        match op_def.op_type {
            OperandType::Set(set) => set
                .iter()
                .find(|t| match **t {
                    OperandType::Reg(_) if op_def.encoding == OperandEncoding::ModRmRm => {
                        buffer.mod_rm_mod.map_or(false, |r| r == 0b11)
                    }
                    OperandType::Bcst(_) => buffer.vex_b.unwrap_or(false),
                    OperandType::Mem(_) => !has_broadcast || !buffer.vex_b.unwrap_or(false),
                    _ => true,
                })
    ...
    fn get_operand_size(op_def: &OperandDefinition, buffer: &InstructionBuffer) -> OperandSize {
        let op_type = InstructionReader::<T>::get_operand_type(op_def, buffer);
        let s = match *op_type {
            OperandType::Mem(Some(s)) | OperandType::Bcst(s) => s,
            _ => op_def.size,
        };
    ...
    fn get_broadcast(
        ...
        buffer.vex_b.map_or(None, |b| {
            if b {
                def.operands
                    .iter()
                    .filter_map(|o| {
                        o.as_ref().and_then(|op| {
                            InstructionReader::<T>::get_broadcast_helper(&op.op_type, op.size)
                        })
                    })
                    .next()
            } else {
    ...
    fn get_broadcast_helper(op_type: &OperandType, op_size: OperandSize) -> Option<BroadcastMode> {
        if let OperandType::Bcst(s) = *op_type {
            Some(
                BroadcastMode::from_multiplier((op_size.bits() / s.bits()) as u8)
                    .expect("Bad instruction definition."),
            )
        } else if let OperandType::Set(set) = *op_type {
            set.iter()
                .filter_map(|i| InstructionReader::<T>::get_broadcast_helper(i, op_size))
                .next()
        } else {
            None
        }
