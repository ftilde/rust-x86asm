use test::{encode32_assert_ambiguous, encode32_helper2, encode32_helper3};
use {
...
fn infer_size_indirect_8bit_instr() {
    encode32_helper2(
        ...
        Operand::Indirect(Reg::EAX, None, None),
        &vec![0x02, 0x00],
    );
}
...
fn infer_vector_size() {
    ...
    ); // VADDPD XMM1, XMM2, XMMWORD PTR [EAX]
    encode32_helper3(
        Mnemonic::VADDPD,
    ...
    ); // VADDPD YMM1, YMM2, YMMWORD PTR [EAX]
    encode32_helper3(
        Mnemonic::VADDPD,
    ...
    ); // VADDPD ZMM1, ZMM2, ZMMWORD PTR [EAX]
}

#[test]
fn notify_ambiguous_size_16_32() {
    encode32_assert_ambiguous(
        Mnemonic::NOT,
        Some(Operand::Indirect(Reg::EAX, None, None)),
        None,
        None,
        None,
    );
