...
pub fn find_instruction_def(
    ...
    INSTR_MNEMONIC_MAP
        ...
        .and_then(|list| {
            let matches = list
                .iter()
                .filter(|enc| enc.matches_instruction(instr, mode));
            let mut sizes = None;
            let mut best = None;
...
pub fn find_instruction_def_by_opcode(
    buffer: &InstructionBuffer,
    mode: Mode,
) -> Result<&'static InstructionDefinition, FindInstructionDefByOpcodeError> {
    let mut matches = INSTR_DEFS.iter().filter(|def| {
        let has_opcode_addend = def.operands.iter().any(|o| {
            o.as_ref()
                .map_or(false, |op| op.encoding == OperandEncoding::OpcodeAddend)
        });
    ...
    if let Some(f) = first {
        ...
        if next.is_some() {
            ...
            // If there's only one match, it's the one we're looking for
            Ok(f)
        }
    } else {
    ...
    fn len(&self) -> u32 {
        ...
        (match self.composite_prefix {
            ...
            + self
                .operands
                .iter()
                .filter_map(|o| o.as_ref().map(|op| op.len()))
                .sum::<u32>()
    }
}
...
impl OperandDefinition {
    pub fn matches_operand(
        ...
        op.map(|o| {
            OperandDefinition::matches_operand_type(&self.op_type, self.size, op, instr_def, instr)
                && if let Operand::Direct(reg) = o {
                    // TODO Cleanup
                    if reg.is_avx() {
                        let code = reg.get_reg_code();
                        if code >= 0x10 {
                            if let Some(CompositePrefix::Evex { .. }) = instr_def.composite_prefix {
                                true
                            } else {
                                false
                            }
    ...
    fn get_real_type(&self, op: &Operand) -> Option<&OperandType> {
        if let OperandType::Set(set) = self.op_type {
            set.iter().find(|s| match **s {
                OperandType::Reg(_) => !op.is_memory(),
                OperandType::Mem(s) => {
                    op.is_memory()
                ...
                OperandType::Bcst(s) => {
                    op.is_memory()
                        && op.size().map_or(true, |sz| {
                            sz == s || sz == OperandSize::Unsized || s == OperandSize::Unsized
                        })
                }
                _ => unimplemented!(),
            })
    ...
    fn matches_operand_type(
        op_type: &OperandType,
        def_size: OperandSize,
        op: &Option<Operand>,
        instr_def: &InstructionDefinition,
        instr: &Instruction,
    ) -> bool {
        fn size_helper(def_size: OperandSize, op: &Option<Operand>) -> bool {
            op.and_then(|o| o.size())
                .map(|s| {
                    s == def_size || s == OperandSize::Unsized || def_size == OperandSize::Unsized
                })
                .unwrap_or(true)
        }
        ...
        match *op_type {
            ...
            OperandType::Mem(s) => {
                op.map(|o| o.is_memory()).unwrap_or(false) && size_helper(s.unwrap_or(def_size), op)
            }
            OperandType::Imm => {
                op.map(|o| o.is_literal() || o.is_offset() || o.is_far())
                    .unwrap_or(false)
                    && size_helper(def_size, op)
            }
            OperandType::Offset => false, //TODO: used to be "unimplemented!", but get's called anyway. "false" seems to be ok so far, but I'm not sure
            ...
            OperandType::Bcst(op_size) => {
                op.map(|o| o.is_memory()).unwrap_or(false)
                    && size_helper(op_size, op)
                    && instr.broadcast.map_or(false, |b_m| {
                        op_size.bits() * b_m.get_multiplier() == def_size.bits()
                    })
            }
            OperandType::Fixed(fixed_op) => {
                op.map(|o| fixed_op.matches_operand(&o)).unwrap_or(true)
            }
            OperandType::Set(types) => types.iter().any(|t| {
                OperandDefinition::matches_operand_type(t, def_size, op, instr_def, instr)
            }),
        }
    }
    ...
    // compare definitions to find the shortest.
    fn len(&self) -> u32 {
        if self.encoding == OperandEncoding::OpcodeAddend {
            return 0;
        }
        ...
        match self.op_type {
            ...
            | OperandType::Mib
            | OperandType::Set(_) => 1,
            OperandType::Imm => self.size.bits() / 8,
            OperandType::Rel(s) => s.bits() / 8,
            OperandType::Fixed(_) => 0,
        }
    }
