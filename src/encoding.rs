    ...
    W: Write,
{
    ...
    buffer.address_size_prefix = def
        .address_size_prefix
        .unwrap_or_else(|| get_address_size_prefix(addr_size, mode));
    ...
    if def.opcode_ext.is_some() {
        buffer.mod_rm_reg = def.opcode_ext;
    }
    if def.fixed_mod_rm_mod.is_some() {
        buffer.mod_rm_mod = def.fixed_mod_rm_mod;
    }
    if def.fixed_mod_rm_reg.is_some() {
        buffer.mod_rm_reg = def.fixed_mod_rm_reg;
    }
...
fn get_operand_size_prefix(behavior: OperandSizePrefixBehavior, mode: Mode) -> bool {
    match behavior {
...
fn get_addr_size(
    ...
    match check_unique::<_, OperandSize, InstructionEncodingError>(
        def.operands
            .iter()
            .zip(instr.operands().iter())
            .filter_map(|(ref op_def, op)| {
                op_def.as_ref().and_then(
                    // TODO Clean up?
                    |_| {
                        get_operand_addr_size(op)
                            .map(|v| v.map(Ok))
...
pub fn get_op_size_64(def: &InstructionDefinition) -> bool {
    if let Some(CompositePrefix::Rex {
        size_64: Some(true),
...
pub fn encode_operand(
    ...
    match def.encoding {
        ...
        OperandEncoding::Mib | OperandEncoding::ModRmRm => {
            encode_rm(buffer, &op.expect("Internal error."), mode)?;
        }
        OperandEncoding::Vex => {
            if let Some(Operand::Direct(reg)) = *op {
        ...
        OperandEncoding::Offset | OperandEncoding::Imm => match op.expect("Internal error.") {
            ...
            Operand::Offset(offset, ..) => {
                let imm_size = if let OperandType::Rel(enc_size) = def.op_type {
                    enc_size
                } else {
                    addr_size
                };
...
fn encode_rm(
    ...
    match *op {
        ...
        Operand::Indirect(base, ..) => encode_indirect(buffer, Some(base), None, None, 0, mode),
        ...
        Operand::Memory(addr, ..) | Operand::Offset(addr, ..) => {
            // TODO Should offset panic or be here?
...
fn encode_indirect(
    ...
    base: Option<Reg>,
    index: Option<Reg>,
    scale: Option<RegScale>,
    displacement: u64,
    mode: Mode,
) -> Result<(), InstructionEncodingError> {
    match mode {
        Mode::Real => encode_indirect_16(buffer, base, index, displacement).or_else(|_| {
            buffer.address_size_prefix = true;
            encode_indirect_32(buffer, base, index, scale, displacement, mode)
        }),
        Mode::Protected => encode_indirect_32(buffer, base, index, scale, displacement, mode)
            .or_else(|_| {
                buffer.address_size_prefix = true;
                encode_indirect_16(buffer, base, index, displacement)
            }),
        Mode::Long => encode_indirect_64(buffer, base, index, scale, displacement).or_else(|_| {
            buffer.address_size_prefix = true;
            encode_indirect_32(buffer, base, index, scale, displacement, mode)
        }),
    }
...
fn encode_indirect_32(
    ...
    if real_scale != RegScale::One && index.is_none() {
        return Err(InstructionEncodingError::InvalidAddressing);
    }

    match base {
        Some(Reg::EAX) | Some(Reg::EBX) | Some(Reg::ECX) | Some(Reg::EDX) | Some(Reg::ESI)
        | Some(Reg::EDI) => {
            match index {
...
fn encode_indirect_64(
    ...
    match base {
        Some(Reg::RAX) | Some(Reg::RBX) | Some(Reg::RCX) | Some(Reg::RDX) | Some(Reg::RSI)
        | Some(Reg::RDI) | Some(Reg::R8) | Some(Reg::R9) | Some(Reg::R10) | Some(Reg::R11)
        | Some(Reg::R12) | Some(Reg::R13) | Some(Reg::R14) | Some(Reg::R15) => {
            match index {
